    ...
    try:
        ...
        def detail(lang):
            if is_series and season:
                return requests.get(
                    f"https://api.themoviedb.org/3/tv/{mid}/season/{season}?api_key={TMDB_API_KEY}&language={lang}"
                ).json()
            else:
                return requests.get(
                    f"https://api.themoviedb.org/3/{url}/{mid}?api_key={TMDB_API_KEY}&language={lang}"
                ).json()
